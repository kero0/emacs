...
(eval-and-compile
  (require 'use-package)
  (advice-add #'use-package-handler/:bind :around #'eval-and-compile))
...
(use-package which-key
  ...
  :commands which-key-add-keymap-based-replacements
  :diminish
  :config
...
(use-package evil
  ...
  (evil-define-key 'normal 'global my/leader-def-prefix my/maps/leader)
  (evil-define-key 'visual 'global my/leader-def-prefix my/maps/leader)
  (global-set-key my/leader-def-prefix-alt my/maps/leader)
...
(use-package popper
  ...
  (defun my/popper-toggle-and-select ()
    (interactive)
    (let ((popper-display-function #'popper-select-popup-at-bottom))
      (popper-toggle)))
  :config
  (my/map-menu! "Popper" "`"
...
(use-package projectile
  ...
  (after! evil
          (keymap-set global-map "<remap> <evil-next-buffer>" #'projectile-next-project-buffer)
          (keymap-set global-map "<remap> <evil-previous-buffer>" #'projectile-previous-project-buffer)
          (keymap-set global-map "<remap> <next-buffer>" #'projectile-next-project-buffer)
          (keymap-set global-map "<remap> <previous-buffer>" #'projectile-previous-project-buffer))
  (my/leader-def
...
(use-package vertico
  ...
  (vertico-mode)
  (after! embark
          (which-key-add-keymap-based-replacements vertico-map
...
(use-package consult
  ...
  (keymap-unset help-map "h")
  (keymap-set help-map "h m" #'consult-man))
...
(use-package embark
  ...
  (keymap-set global-map "<remap> <describe-bindings>" #'embark-bindings)
  (add-to-list 'display-buffer-alist
               '("\\`\\*Embark Collect \\(Live\\|Completions\\)\\*"
                 nil
                 (window-parameters (mode-line-format . none))))
