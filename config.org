...
:DOC-CONFIG:
#+property: header-args :emacs-lisp :tangle init.el :results output silent :noweb yes :lexical t
#+startup: fold
:END:
* Setup
...
(use-package evil-args
  ...
  :general (:states 'normal
		    ...
		    ;; overriding visual line motions
		    "gk" 'evil-jump-out-args
		    'evil-textobj-
		    "gj" 'evil-jump-out-args)
...
(use-package evil-textobj-anyblock
  :after evil
  :general (:states 'visual
		    "ib" 'evil-textobj-anyblock-inner-block
		    "ab" 'evil-textobj-anyblock-a-block
		    "iq" 'evil-textobj-anyblock-inner-quote
		    "aq" 'evil-textobj-anyblock-a-quote)
  :general (:states 'normal
		    "]b" 'evil-textobj-anyblock-forward-any-block-start
		    "[b" 'evil-textobj-anyblock-backward-any-block-start
		    "]q" 'evil-textobj-anyblock-forward-any-quote-start
		    "[q" 'evil-textobj-anyblock-backward-any-quote-start
		    "]B" 'evil-textobj-anyblock-forward-any-block-end
		    "[B" 'evil-textobj-anyblock-backward-any-block-end))
(use-package evil
  :config
...
(use-package string-inflection
  :after evil
  :general (my/general-global-menu! "c ~" "naming convention"
...
(use-package projectile
  ...
  :general (my/leader-def "p" '(:keymap projectile-command-map :wk "Projectile"))
  :general (:keymaps 'projectile-command-map "a" 'projectile-add-known-project))
...
(use-package embark
  ...
  (add-hook 'embark-collect-mode-hook #'embark-visualize-sort)
...
(use-package haskell-mode
  ...
  (eval-after-load 'projectile (progn
				 (add-to-list 'projectile-other-file-alist '("hs" "lhs"))
...
(defun my/get-mail-dir ()
  (let ((dir (expand-file-name "mail/" (getenv "XDG_DATA_HOME"))))
    (unless (file-exists-p dir)
      (error "Mail directory %s does not exist" dir))
    dir))
(defvar my/mail-dir (my/get-mail-dir))
...
(use-package mu4e
  ...
  (setq mu4e-maildir my/mail-dir)

  ;; Context switching for multiple accounts
  (defun my/create-mu4e-context (name address signature server)
    (make-mu4e-context
     :name name
  ...
  (setq mu4e-contexts
	(list
	 ...
	 (my/create-mu4e-context "kirolsb5-gmail" "kirolsb5@gmail.com" (my/gmail-signature "kirolsb5") "smtp.gmail.com")
	 (my/create-mu4e-context "bakheakm-udmercy" "bakheakm@udmercy.ed" (my/udmercy-signature) "smtp.office365.com"))))
...
;; (add-to-list 'org-latex-package-alist '("" "fancyhdr"))
;; (add-to-list 'org-latex-package-alist '("" "siunitx"))
...
(use-package org-roam
  ...
  :general (my/general-global-menu! "Roam" "nr"
	     ...
	     "t" '(org-roam-dailies-find-today :which-key "today")
	     "y" '(org-roam-dailies-find-yesterday :which-key "yesterday")
	     "r" '(org-roam-dailies-find-tomorrow :which-key "tomorrow")
	     "d" '(org-roam-dailies-find-date :which-key "date")
	     "c" '(org-roam-dailies-capture-today :which-key "capture today")
	     "C" '(org-roam-dailies-capture-tomorrow :which-key "capture tomorrow")
	     "D" '(org-roam-dailies-capture-date :which-key "capture date")
	     "l" '(org-roam-buffer-toggle-display :which-key "toggle display")
	     "L" '(org-roam-buffer-display-dedicated :which-key "display dedicated")
	     "s" '(org-roam-db-sync :which-key "sync")
	     "S" '(org-roam-server-mode :which-key "server")))
#+end_src
*** Templates
:PROPERTIES:
:header-args: :tangle yes :noweb-ref roam-templates
:END:
**** Religious
#+begin_src emacs-lisp
