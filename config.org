...
(use-package lspce
  ...
  (setq lspce-inherit-exec-path t)
...
(use-package nix-ts-mode
  ...
  (after! apheleia
          ...
          (setf (alist-get 'nix-ts-mode apheleia-mode-alist)
                '(nixfmt)))
  (after! lspce
          (append! lspce-server-programs '("nix" "nil" ""))
...
(use-package python
  ...
  :config
  (append! lspce-server-programs '("python" "pylsp" "" ))
  (after! apheleia
...
(use-package flymake-ruff
  :unless noninteractive
  :hook (lspce-mode . flymake-ruff-load))
...
(unless noninteractive
  (use-package c-ts-mode
    ...
    (("\\.c\\'" . c-ts-mode)
     ("\\.h\\'" . c-or-c++-ts-mode))
    :hook (c-ts-mode . lspce-mode)
    :hook (c-ts-mode . my/treesit-mode)
    :hook (c-ts-mode . (lambda () (run-hooks 'c-mode-hook)))
    :config
    (append! lspce-server-programs '("C" "clangd" "--all-scopes-completion --clang-tidy --enable-config --header-insertion-decorators=0"))
...
(use-package rustic
  ...
  :config
  (append! lspce-server-programs '("rust"  "rust-analyzer" "" lspce-ra-initializationOptions))
...
(use-package haskell-mode
  ...
  (add-hook 'haskell-mode-hook 'lspce-mode)
...
(use-package julia-mode
  :mode ("\\.jl\\'" . julia-mode)
  :config
  (add-hook 'julia-mode-hook #'lspce-mode)
...
(use-package kotlin-ts-mode
  :hook (kotlin-ts-mode . lspce-mode)
  :mode "\\.kts?\\'"
  :config
