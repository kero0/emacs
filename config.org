...
(use-package pretty-mode
  ...
  :defer 5
  :config
  (pretty-activate-groups '(:function
...
(use-package popper
  ...
  (defun my/popper-toggle-latest ()
    "Toggle popper latest."
    (interactive)
    (if popper-open-popup-alist
	(progn
	  (let ((buffer (cdr (car popper-open-popup-alist)))
		(window (car (car popper-open-popup-alist))))
	    (kill-buffer buffer)
...
(use-package workgroups2
  :unless noninteractive
  :general (my/general-global-menu! "Workgroups" "w"
             "w" '(wg-switch-to-workgroup :wk "Switch")
             "n" '(wg-create-workgroup :wk "New")
             "r" '(wg-rename-workgroup :wk "Rename")
             "d" '(wg-delete-workgroup :wk "Delete")
             "s" '(wg-save-session :wk "Save")
             "l" '(wg-load-session :wk "Load"))
  :init
  (setq wg-session-file (concat my/cache-dir "workgroups")
...
(use-package dirvish
  :unless noninteractive
  :general (my/general-global-Open "d" '(dirvish :wk "dirvish"))
  :init
...
(use-package lsp-mode
  ...
  :general (my/general-global-Toggle "l" '(lsp :wk "LSP"))
  :config
