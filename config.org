...
(use-package recentf
  ...
  (let ((home (getenv "HOME")))
    (dolist (dir `("/nix/store/.*"
      ...
      (add-to-list 'recentf-exclude (string-replace home "~" dir))))
...
#+begin_src emacs-lisp
(after! tramp
        (add-hook 'find-file-hook (defun my/disable-vc-remote ()
        ...
        (add-hook 'tramp-file-local-hook
                  (lambda () (vc-mode -1))))
#+end_src
* Keybindings
** Basics
...
** Fonts
#+begin_src emacs-lisp
(defconst my/font/name "JetBrainsMono Nerd Font Mono")
...
(my/leader-def
 "U" '("Insert Unicode char (by name)" . insert-char)
 "=" '("Font size" . global-text-scale-adjust))
(my/map-menu! "Input-Language" "l"
...
(use-package modus-themes
  ...
  :config
  (defun my/set-theme-per-frame (frame)
    (with-selected-frame frame
      (unless (display-graphic-p frame)
        (make-local-variable 'modus-themes-common-palette-overrides)
        (append! modus-themes-common-palette-overrides '(bg-main unspecified)))
      (load-theme 'modus-operandi t)))
  (my/set-theme-per-frame (selected-frame))
  (my/map-Toggle
...
** Projectile
#+begin_src emacs-lisp
(defvar my/ignore-projects '("^/sudo:" "^/docker:" "^/nix/store"))
(use-package projectile
  ...
  (eval-and-compile
    (setopt projectile-ignored-project-function
            (defun my/ignore-projects (project-root)
                "Ignore matching regexes in `my/ignore-projects'."
  ...
  (after! evil
          (bind-keys
           ...
           ([remap evil-prev-buffer] . projectile-previous-project-buffer))
          (evil-define-key 'normal 'global
            "] b" 'projectile-next-project-buffer
            "[ b" 'projectile-previous-project-buffer))
  (my/map-Buffer
...
(use-package persp-mode
  ...
  (persp-save-dir (expand-file-name ".persp-confs/" my/local-dir))
...
(use-package evil-textobj-tree-sitter
  ...
  (which-key-add-keymap-based-replacements +tree-sitter-inner-text-objects-map
    "a" `("argument" . ,(evil-textobj-tree-sitter-get-textobj "parameter.inner"))
  ...
  (which-key-add-keymap-based-replacements +tree-sitter-outer-text-objects-map
    "a" `("argument" . ,(evil-textobj-tree-sitter-get-textobj "parameter.outer"))
...
(use-package magit
  ...
  (magit-bury-buffer-function
   (defun my/kill-window-and-buffer (&optional kill-buffer)
     "Always kill the current buffer. If multiple windows are open then also kill the window."
     (interactive "P")
     (if (one-window-p)
...
(use-package eldoc-box
  ...
  :custom
  (eldoc-box-max-pixel-height #'my/eldoc-box-height)
  :init
  (defun my/eldoc-box-height ()
    (round (* 0.3 (frame-pixel-height))))
  :config
  (my/map-Toggle "d" '("Doc tooltip" . eldoc-box-hover-mode)))
#+end_src
* Languages
** Nix
#+begin_src emacs-lisp
...
(use-package plantuml-mode
  ...
  (plantuml-executable-path (or (executable-find "plantuml") "/usr/bin/plantuml"))
  (org-plantuml-exec-mode 'plantuml)
  (org-plantuml-executable-path plantuml-executable-path)
  :config
  (defun my/plantuml-capf-fix ()
    (interactive)
    (kill-local-variable 'completion-at-point-functions)
    (add-to-list 'completion-at-point-functions #'plantuml-completion-at-point-function))

  (add-hook 'plantuml-mode-hook #'my/plantuml-capf-fix)
...
(use-package ox-reveal
  :after org
  :custom
  (org-reveal-root (getenv "ORG_REVEAL_ROOT"))
  (org-reveal-hlevel 1)
...
(use-package org
  ...
  (setq org-confirm-babel-evaluate nil
        ...
        org-babel-header-args:emacs-lisp '((:session "none"))
  ...
  (add-to-list 'org-latex-classes
               '("exam"
                 ...
                 ("\\begin{parts} %s"
                  ...
                  "\\begin{parts} %s"
                  "\\end{parts}")))
  (defun my/org-latex-item (item contents info)
    "Export ITEM as exam-class choices."
    (let* ((parent (org-element-property :parent item))
           (env (org-export-read-attribute :attr_latex parent :environment))
           (checkbox (org-element-property :checkbox item))
           (text (org-trim (or contents ""))))
      (if (member env '("choices" "oneparchoices" "checkboxes"))
          (concat
           (cond
            ((eq checkbox 'on) "\\CorrectChoice ")
            ((eq checkbox 'trans) "\\CorrectChoice ")
            (t "\\choice "))
           text)
        nil)))

  (advice-add 'org-latex-item :before-until #'my/org-latex-item)

  (defun my/quiz-export (add-answers-p)
    "Export the current Org quiz with or without answers."
    (interactive "P")
    (let* ((src-buf      (current-buffer))
           (src-file     (buffer-file-name))
           (keywords     (with-current-buffer src-buf
                           (org-element-map (org-element-parse-buffer)
                               '(keyword)
                             (lambda (k) (cons (org-element-property :key k)
                                               (org-element-property :value k))))))
           (orig-title   (or (cdr (assoc "TITLE" keywords)) "Quiz"))
           (orig-fname   (or (cdr (assoc "EXPORT_FILE_NAME" keywords))
                             (file-name-sans-extension
                              (file-name-nondirectory src-file))))
           (orig-class   (or (cdr (assoc "LATEX_CLASS" keywords)) "exam"))
           (orig-opts    (or (cdr (assoc "LATEX_CLASS_OPTIONS" keywords)) "[]"))

           (new-title    (if add-answers-p
                             (concat orig-title " (answers)")
                           orig-title))
           (new-fname    (if add-answers-p
                             (concat orig-fname " answers")
                           orig-fname))
           (new-opts     (if add-answers-p
                             (if (string-match-p "\\[.*answers.*\\]" orig-opts)
                                 orig-opts
                               (replace-regexp-in-string
                                "\\[\\(.*\\)\\]" "[\\1,answers]" orig-opts))
                           (replace-regexp-in-string ",answers" ""
                                                     (replace-regexp-in-string "\\[answers\\]" "[]" orig-opts))))
           (tmp-buf      (generate-new-buffer "*quiz-export*")))
      (with-current-buffer tmp-buf
        (insert (format "#+LATEX_CLASS: %s\n" orig-class)
                (format "#+LATEX_CLASS_OPTIONS: %s\n" new-opts)
                (format "#+EXPORT_FILE_NAME: %s\n" new-fname)
                (format "#+TITLE: %s\n" new-title)
                (format "#+INCLUDE: \"%s::Quiz\" :minlevel 1 :only-contents t\n" src-file))
        ;; Strip every drawer at level-0 *in the included file*
        (add-hook 'org-export-before-processing-hook
                  (lambda (_)
                    (org-element-map (org-element-parse-buffer) 'drawer
                      (lambda (d)
                        (when (= (org-element-property :level d) 0)
                          (org-element-extract-element d)))))
                  nil t)
        (org-latex-export-to-pdf)
        (kill-buffer))))

  (defun my/quiz-export-answers ()   (interactive) (my/quiz-export t))
  (defun my/quiz-export-no-answers () (interactive) (my/quiz-export nil)))
#+end_src
*** =ox-html=
#+begin_src emacs-lisp
(use-package ox-html
